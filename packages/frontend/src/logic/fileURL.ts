...
export function getTrackFileURL(file: TrackFile): string {
  return `${CDN_ENDPOINT}/files/${file.region}/audio/${file.userId}/${file.id}${file.extension}`;
}

export function getImageFileURL(file: ImageFile): string {
  return `${CDN_ENDPOINT}/files/${file.region}/image/${file.userId}/${file.id}${file.extension}`;
}
