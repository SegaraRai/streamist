...
const WS_ORIGIN_FOR_SERVER = process.env.WS_ORIGIN_FOR_SERVER ?? '';
...
const SECRET_WS_JWT_SECRET = process.env.SECRET_WS_JWT_SECRET ?? '';
...
const SECRET_WS_AUTH_TOKEN = process.env.SECRET_WS_AUTH_TOKEN ?? '';
...
const SECRET_INVOKE_TRANSCODER_GCR_SERVICE_ACCOUNT =
  process.env.SECRET_INVOKE_TRANSCODER_GCR_SERVICE_ACCOUNT ?? '';
const SECRET_INVOKE_TRANSCODER_LAMBDA_ACCESS_KEY_ID =
  process.env.SECRET_INVOKE_TRANSCODER_LAMBDA_ACCESS_KEY_ID ?? '';
const SECRET_INVOKE_TRANSCODER_LAMBDA_SECRET_ACCESS_KEY =
  process.env.SECRET_INVOKE_TRANSCODER_LAMBDA_SECRET_ACCESS_KEY ?? '';
const HCAPTCHA_SITE_KEY_FOR_REGISTRATION =
  process.env.HCAPTCHA_SITE_KEY_FOR_REGISTRATION ?? '';
const SECRET_HCAPTCHA_SECRET_KEY = process.env.SECRET_HCAPTCHA_SECRET_KEY ?? '';

if (!WS_ORIGIN_FOR_SERVER) {
  throw new Error('WS_ORIGIN_FOR_SERVER is not defined');
}
...
if (!SECRET_WS_JWT_SECRET) {
  throw new Error('SECRET_WS_JWT_SECRET is not defined');
}
...
if (!SECRET_INVOKE_TRANSCODER_GCR_SERVICE_ACCOUNT) {
  throw new Error(
    'SECRET_INVOKE_TRANSCODER_GCR_SERVICE_ACCOUNT is not defined'
  );
}

if (!SECRET_INVOKE_TRANSCODER_LAMBDA_ACCESS_KEY_ID) {
  throw new Error(
    'SECRET_INVOKE_TRANSCODER_LAMBDA_ACCESS_KEY_ID is not defined'
  );
}

if (!SECRET_INVOKE_TRANSCODER_LAMBDA_SECRET_ACCESS_KEY) {
  throw new Error(
    'SECRET_INVOKE_TRANSCODER_LAMBDA_SECRET_ACCESS_KEY is not defined'
  );
}
...
if (!SECRET_WS_AUTH_TOKEN) {
  throw new Error('SECRET_WS_AUTH_TOKEN is not defined');
}

if (!IS_DEV && !HCAPTCHA_SITE_KEY_FOR_REGISTRATION) {
  throw new Error('HCAPTCHA_SITE_KEY_FOR_REGISTRATION is not defined');
}

if (!IS_DEV && !SECRET_HCAPTCHA_SECRET_KEY) {
  throw new Error('SECRET_HCAPTCHA_SECRET_KEY is not defined');
}
